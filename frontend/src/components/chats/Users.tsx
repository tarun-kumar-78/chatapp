...
const Users = () => {
    ...
    const [logout, setUserLogout] = useState<boolean>(false);
    ...
    const getLatestMessages = async (conversationId: string) => {
        try {
            const response = await api.get(`/api/message/getMessages/${conversationId}`);
    ...
    const handleUserTabClick = async (user: User) => {
        ...
        const messages = await getLatestMessages(conversationId);
        ...
        <>
            <div className="border bg-gray-300/30 w-full md:w-[20%] min-w-70 p-4">
                <div className="flex justify-between items-center h-12">
                    <div className="flex items-center gap-3">
                        <img src={user?.avatar || img} alt="profile image" className="h-9 w-9 sm:w-10 sm:h-10 rounded-full" />
                        <div className="">
                ...
                <div className='flex justify-between flex-col h-[80%]'>
                    ...
                    <div className='flex items-center justify-center cursor-pointer'>
                        <div className='flex gap-3 bg-black p-1 text-white items-center rounded-sm' onClick={() => setUserLogout(true)}>
                            <span>Logout</span>
            ...
            <AlertDialog open={logout} >
                <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                        <AlertDialogCancel onClick={() => setUserLogout(false)}>Cancel</AlertDialogCancel>
